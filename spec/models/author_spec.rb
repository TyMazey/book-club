  ...
  describe 'instance methods' do
    describe '.best_review' do
      it 'returns the top review for a specific author' do
        ...
        good = book.reviews.create(title: 'great', description: 'great', rating: 5, user: user).attributes
      ...
      it 'returns no reviews if an author has none' do
        author = Author.create(name: "bob")
        book = author.books.create(title: "book 1", pages: 100, year_published: 1901, thumbnail: "picture url")
        blank_review = {'title' => 'No Reviews Yet', 'rating' => '', 'user' => ''}

        expect(author.best_review).to eq(blank_review)
      end
    end
  end
end
